...
\label{sec-6}
  ...
  \label{resul}
\subsection{Rendimiento de las implementaciones paralelas de red neuronal}
\label{sec-6-1}

  \label{resulImpl}
\subsubsection{Acumulación de resultados}
\label{sec-6-1-1}
...
\begin{figure}[htb]
\centering
\includegraphics[width=\textwidth]{./img/Buffer_activation.png}
\caption{\label{grafImplActivation}Método que ejecuta la función de activación para las salidas de una capa (Buffer::activation).}
\end{figure}
\newpage
...
\end{figure}
\newpage
\subsubsection{Copiar desde y hacia Interfaz}
\label{sec-6-1-5}
...
%
% No he puesto todas las referencias que tenÃ­a porque tenÃ­a demasiadas y no tengo muy claro quÃ© tengo que hacer cuando unos se citan a otros. 
% Por ejemplo, he dejado muchas citas que Bertona hacÃ­a de otros y no sÃ© si eso es correcto. Yao en una sÃ³la frase referencia a muchÃ­sima gente y no sÃ© muy bien quÃ© deberÃ­a hacer.
%Desde luego, he aprendido que la bibliografÃ­a es mejor prepararla sobre la marcha que al toda final.
%
