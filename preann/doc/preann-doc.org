...
\newpage
* REVISAR Manual del programador
...
- =Board(unsigned size, BufferType bufferType) := Crea un tablero cuadrado de tamaño =size * size= con todas las casillas vacías. El /tipo de neurona/ se le pasa para poder probar por separado redes similares con diferentes representaciones internas. La interfaz usará el BufferType parámetro.
...
- =ReversiTask(unsigned size, BufferType bufferType, unsigned numTests = 1) := Se inicializa la tarea reversi con un tablero cuadrado con lado =size= de la clase ReversiBoard. Como se explicó en esa clase, la Interface usará el BufferType parámetro.
...
Se realizan dos pruebas para esta función: una variando el tamaño de la capa de entrada y otra variando el de salida.

**** MODIFICAR En función del tamaño de las entradas
...
**** HACER En función del tamaño de las salidas
