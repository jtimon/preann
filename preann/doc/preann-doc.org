...
En esta secci�n se analizan resultados de rendimiento en tiempo de ejecuci�n.

*** Rendimiento de las implementaciones paralelas de red neuronal
  #+LaTeX: \label{rendImpl}

En esta secci�n se presentan los resultados en tiempo de ejecuci�n de los m�todos que se ejecutan de forma diferente con cada implementaci�n.

**** Acumulaci�n de resultados

En la figura \ref{grafImplCalculate}, puede apreciarse que la implementaci�n con instrucciones SSE2 de ensamblador para utilizar el coprocesador XMM es muy superior a la implementaci�n de referencia en lenguaje C, especialmente para los tipos de Buffer BIT y SIGN.

#+CAPTION:    M�todo de acumulaci�n de resultados (Connection::calculateAndAdd).
...
De la figura \ref{grafImplActivation} se extraen varias conclusiones. No mucha diferencia de rendimiento entre las implementaciones y pr�cticamente ninguna cuando las neuronas son de tipo FLOAT. Esto era de esperar porque ambas implementaciones son id�nticas: no se han hecho optimizaciones SSE2 para la funci�n de activaci�n.

Para los tipos BIT y SIGN, la implementaci�n en C es ligeramente m�s r�pida que para el tipo FLOAT y la  escrita en SSE2 es ligeremante m�s lenta que la FLOAT. En la implementaci�n SSE2 de BIT y SIGN no se ponen los bits en el orden normal, si no en la representaci�n especial de SSE2, para que el m�todo de acumulaci�n de resultados pueda ser �ptimo.

#+CAPTION:    M�todo que ejecuta la funci�n de activaci�n para las salidas de una capa (Connection::activation).
...
No hay una diferencia sustancial de rendimiento entre las diferentes implementaciones y tipos de neuronas para el operador g�n�tico de crossover, como se muestra en la figura \ref{grafImplCrossover}. Como el m�todo de activaci�n, la cruza de SSE2 se ve ligeramente penalizada al traducir el vector de bits con los pesos a cruzar para que tenga se adapte a la posici�n especial de los pesos en SSE2.

#+CAPTION:    M�todo que implementa el operador de crossover para dos conexiones dadas (Connection::crossover).
...
De la figura \ref{grafImplMutation} se puede deducir que el operador de mutaci�n tiene un coste m�nimo y constante respecto al tama�o de la conexi�n para todas las implementaciones.

#+CAPTION:    M�todo que implementa el operador gen�tico de mutaci�n (Connection::mutate).
...
De manera similar a la operaci�n de mutaci�n, el olvido (figura \ref{grafImplReset}) tiene un coste constante y m�nimo.

#+CAPTION:    M�todo que implementa el operador gen�tico de olvido (Connection::reset).
#+LABEL:      grafImplReset
#+ATTR_LaTeX: width=\textwidth
...
Estos m�todos se usan para recibir entradas y sacar salidas al exterior de la red. Las interfaces sirven para independizar la red de la implementaci�n concreta escogida. Cada implementaci�n debe mapear correctamente desde la representaci�n gen�rica (Interfaz) hacia su propia representaci�n interna (figura \ref{grafImplCopyFrom}) de los datos y viversa (figura \ref{grafImplCopyTo}).

#+CAPTION:    M�todo que implementa la copia a un Buffer con una implementaci�n determinada desde un Buffer gen�rico de la clase Interfaz (Buffer::copyFromInterface).
#+LABEL:      grafImplCopyFrom
#+ATTR_LaTeX: width=\textwidth
[[./img/Buffer_copyFromInterface.png]]

En ambos casos se observa que el coste es muy peque�o excepto para la implementaci�n SSE2 de los tipos BIT y SIGN, por razones similares a las de Activaci�n.

#+CAPTION:    M�todo que implementa la copia de un Buffer con una implementaci�n determinada a un Buffer gen�rico de la clase Interfaz (Buffer::copyToInterface).
#+LABEL:      grafImplCopyTo
#+ATTR_LaTeX: width=\textwidth
[[./img/Buffer_copyToInterface.png]]
\newpage

*** Rendimiento de los diferentes operadores gen�ticos
   #+LaTeX: \label{rendOperadores}

En esta secci�n se cronometran los operadores de selecci�n. Algunos ya se hab�an analizado en la secci�n \ref{rendImpl}, pero a un nivel m�s bajo, del que dependen las implementaciones. Ahora lo que nos interesa es principalmente el coste de los operadores dependiendo de las diferentes formas de usarlo a m�s alto nivel. Nos abstraemos por ello de la implementaci�n concreta y usamos la implementaci�n C para todas estas pruebas.
...
En esta secci�n se muestran resultados de aprendizaje en t�rminos del mejor fitness de la poblaci�n en cada generaci�n.
