...
//        mutationLoop->addEnumLoop(MA_DISABLED, NULL);
        mutationLoop->addEnumLoop(MA_PER_INDIVIDUAL, new RangeLoop(Population::MUTATION_NUM, 1, 4, 2));
//        mutationLoop->addEnumLoop(MA_PROBABILISTIC, new RangeLoop(Population::MUTATION_PROB, 0.05, 0.2, 0.1));
        averageLoop.addInnerLoop(mutationLoop);


        plotter.plotTaskAveraged("Crossover", &linesLoop, &averageLoop);
