...
typedef enum {BT_FLOAT, BT_BIT, BT_SIGN, BT_BYTE} BufferType;
#define BUFFER_TYPE_DIM 4
typedef enum {FT_IDENTITY, FT_BINARY_STEP, FT_BIPOLAR_STEP, SIGMOID, FT_BIPOLAR_SIGMOID, FT_HYPERBOLIC_TANGENT} FunctionType;
#define FUNCTION_TYPE_DIM 6
typedef enum {IT_C, IT_SSE2, IT_CUDA, IT_CUDA_REDUC, IT_CUDA_INV} ImplementationType;
#define IMPLEMENTATION_TYPE_DIM 5
typedef enum {SA_ROULETTE_WHEEL, SA_RANKING, SA_TOURNAMENT, SA_TRUNCATION} SelectionAlgorithm;
#define SELECTION_ALGORITHM_DIM 4
typedef enum {CL_WEIGH, CL_NEURON, CL_NEURON_INVERTED, CL_LAYER} CrossoverLevel;
