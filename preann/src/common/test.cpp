...
EnumType Test::enumTypeAtPos(unsigned pos)
{
    ...
    if (pos >= enumerations.size()) {
        string error = "Test::enumTypeAtPos : cannot access the pos "
                + to_string(pos) + " there's only " + to_string(
                enumerations.size()) + " enumerations.";
        throw error;
    }
...
EnumIterConfig* Test::getEnumConfig(EnumType enumType)
{
    if (!enumMap.count(enumType)) {
        string error =
                "Test::getEnumConfig : The test has no defined vector for enum "
                        + Enumerations::enumTypeToString(enumType);
        throw error;
    }
...
{
    if (pos >= enumerations.size()) {
        string error = "Test::getEnumConfigAtPos : cannot access the pos "
                + to_string(pos) + " there's only " + to_string(
                enumerations.size()) + " enumerations.";
        throw error;
    }
...
unsigned Test::getEnum(EnumType enumType)
{
    ...
    if (index >= vectorSize) {
        string error = "Test::getEnum : The index points to "
                + to_string(index) + " but the vector has only " + to_string(
                vectorSize) + " values.";
        throw error;
    }
...
void setToIteratorConfig(IteratorConfig &iteratorConfig, float min, float max,
        float increment)
{
...
void* Test::getVariable(std::string key)
{
    if (!variables.count(key)) {
        std::string error = " Test::getVariable : variable \"" + key
                + "\" not found.";
        throw error;
    }
...
std::string Test::getCurrentState()
{
    ...
    FOR_EACH(it, enumMap) {
        if (getEnumConfigAtPos(it->second)->valueVector.size() > 1) {
            state += "_"
                    + Enumerations::toString(it->first, getEnum(it->first));
        }
    }
