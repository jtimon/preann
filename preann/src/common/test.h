...
public:
    ...
    template<class classTempl>
        void loopFunction(void(*action)(classTempl(*)(Test*), Test*),
                classTempl(*f)(Test*), string& testedMethod)
        {
            resetLoops();
            try {
                ...
                cout << " While looping " << testedMethod << " State: "
    ...
    template<class classTempl>
        void forEnumsIters(void(*action)(classTempl(*)(Test*), Test*),
                classTempl(*f)(Test*), unsigned enu)
        {
            if (enu == enumerations.size()) {
                ...
                for (enumConfig->index = 0; enumConfig->index
                        < enumConfig->valueVector.size(); ++enumConfig->index) {
    ...
    template<class classTempl>
        void forEnums(void(*action)(classTempl(*)(Test*), Test*),
                classTempl(*f)(Test*), unsigned enu)
        {
            if (enu == enumerations.size()) {
                //    		printCurrentState();
    ...
    template<class classTempl>
        void forIters(void(*action)(classTempl(*)(Test*), Test*),
                classTempl(*f)(Test*), unsigned iter)
        {
