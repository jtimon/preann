...
extern "C" void SetInputsInDevice(struct_Layer* d_layer, void** inputs){
	...
	unsigned numberInputLayers = d_layer->numberInputLayers;
	for(unsigned i=0; i<numberInputLayers; i++){
		d_layer->inputNeurons[i] = inputs[i];
	}
...
*/
	//TODO comprobar que en modo no emulado esto siguiente sustituye a lo anterior
	...
	if (outputType == BIT) {
		if (tid == 0) {
			...
			if (sdata[0] - layer->thresholds[outputNeuron] > 0){
				...
				//(((unsigned*)(layer->outputNeurons))[outputNeuron / BITS_PER_UNSIGNED]) =
				//		(((unsigned*)(layer->outputNeurons))[outputNeuron / BITS_PER_UNSIGNED]) | mask;
			} else {
