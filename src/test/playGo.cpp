...
{
    cout << "=== PREANN Go Persistent Training ===" << endl << endl;

    try {
        // Parse positional arguments: ./bin/playGo.exe <generations> <checkpoint>
        if (argc < 3) {
        ...
        const char* saveFile = "output/data/go_persist.pop";

        // Create Go task (5x5 board for faster testing, 1 test game)
        cout << "Creating Go task (5x5 board, 1 test game)..." << endl;
        GoTask goTask(5, BT_BIT, 1);

        // Set up population parameters (minimal population for faster testing)
        ...
        params.putNumber(Population::SIZE, 2);
        params.putNumber(Population::NUM_SELECTION, 1);
        params.putNumber(Population::NUM_CROSSOVER, 1);
        params.putNumber(Population::NUM_PRESERVE, 1);
        ...
        if (fileExists(saveFile)) {
            ...
            population = new Population(&goTask, 2);
            ...
            cout << "Creating population of 2 individuals (this may take a while)..." << endl;
            population = new Population(&goTask, example, 2, 5.0);
            population->setParams(&params);

            startGeneration = 0;
            cout << "Created population of 2 individuals" << endl;
        }
        ...
        for (unsigned gen = 0; gen < generations; gen++) {
            ...
            if (population->getBestIndividual()->getFitness() >= goTask.getGoal()) {
